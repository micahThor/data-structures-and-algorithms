...
const count = (target, input) => {
  ...
  input.map( (value, index, array) => {
    ...
    let targetInsideArrayCounter =  array[index].filter( val => {
      return val === target;
    });

    counter += targetInsideArrayCounter.length;
    return targetInsideArrayCounter;
...
const totalSum = (input) => {
  ...
  input.reduce( (accum, current, idx, innerArray) => {

    let innerCounter = innerArray[idx].reduce(reducer);
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let filteredByNotDivisibleByFive = input.filter( (val, idx, arr) => {
    
    let innerArray = arr[idx].filter( (val2, idx2, arr2) => {
      if (Number.isInteger(val2) && val2 % 5 === 0) {
        return val2;
      }
    });

    let outerArray = innerArray.map( val => Math.pow(2, val));

    accumulator.push(outerArray);
    return outerArray;
...
let findMaleAndFemale = (data) => {
  ...
  let arrayOfPeople = data.filter( val => {
    if (val.gender === 'male' || val.gender === 'female') {
      return val;
    }
  });

  let arrayOfNames = arrayOfPeople.map( value => {
    return value.name;
  });

  let names = arrayOfNames.reduce( (accum, curr) => {
    return accum + ' and ' + curr;
  });
...
let findShortest = (data) => {
  ...
  let arrayOfNames = data.map( value => {
    return value.name;
  });

  let currentHighestLength = arrayOfNames[0].length;

  let shortestName = arrayOfNames.filter( (val, idx, arr) => {
    
    if (val.length <= currentHighestLength) {
      currentHighestLength = val.length;
      return val;
    }

    
  });
  return shortestName[shortestName.length - 1];
};
