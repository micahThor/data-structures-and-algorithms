...
const citiesAtoJ = (arr) => {
  let arrayOfCities = [];
  let regex = /^[A-J]+[a-z]{1,}/g;
  
  arr.forEach( word => {
    if (word.match(regex) !== null) {
      //console.log(word.match(regex));
      arrayOfCities.push(word.match(regex)[0])
    }
  });
...
const matchMonth = (input) => {
  ...
  if (word.match(/\b(oct)\b/g)) {
    return true;
  } else if (word.match(/\b(october)\b/g)) {
    return true;  
  }  else if (word.match(/\b(Oct)\b/g)) {
    return true;
  } else if (word.match(/\b(October)\b/g)) {
    return true;
  } else {
