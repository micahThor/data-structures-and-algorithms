...
const countNumberOfElements = (arr) => {
  return arr.reduce((accumulator, currentVal) => accumulator + 1);
};
...
const reversedString = (str) => {
  ...
  return arr.reduce((accum, val, idx, arr) => {

    if (arr.length - idx - 2 >= 0) {
      accum += arr[arr.length - idx - 2];
    }
    return accum;
...
Used code from this website.  I was close to this solution on paper.  This website really broke it down for me to push past the last piece which was placing the division just outside the reduce method */


const calculateAverage = (arr) => {
  
  return arr.reduce( (accum, val) => (accum + val) ) / arr.length;
};
